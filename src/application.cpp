...
Application::Application(GLFWwindow *window) : m_window(window) {
	...
	m_model.color = glm::vec3(0.5, 0.5, 1);
...
void Application::render() {
    ...
    mat4 rotatePitch = glm::rotate(m_model.modelTransform, (float) radians(m_pitch), vec3(1.0, 0.0, 0.0));
    mat4 rotateYaw = glm::rotate(m_model.modelTransform, (float) radians(m_yaw), vec3(0.0, 1.0, 0.0));
	mat4 view = translate(mat4(1), vec3(0, -5, -m_distance)) * rotateYaw * rotatePitch; // TODO replace view matrix with the camera transform
...
void Application::renderGUI() {
	...
	ImGui::Begin("Environment Options", 0);
    ...
    ImGui::SliderFloat("Specular Strength", &m_model.specularStrength, 0, 100, "%.3f");
...
void Application::mouseButtonCallback(int button, int action, int mods) {
    if (button == 0 && action == 1){ mouse_press = true; }
    else if (button == 0 && action == 0){ mouse_press = false; }
}
